...
from dqf import DataProfiler, DataValidator, ValidationRule
from dqf.validator import Severity


# Page config
st.set_page_config(page_title="Data Quality Dashboard", page_icon="📊", layout="wide", initial_sidebar_state="expanded")
...
uploaded_file = st.sidebar.file_uploader(
    "Upload Dataset", type=["csv", "parquet"], help="Upload your dataset for quality analysis"
)
...
if st.session_state.df is None:
    ...
    if st.button("🚀 Run Quality Analysis", type="primary"):
        with st.spinner("Analyzing data quality..."):
            ...
            if run_validation:
                ...
                if rules:
                    st.session_state.validation_result = validator.validate(df, rules, dataset_name="uploaded_data")
    ...
    if st.session_state.profile or st.session_state.validation_result:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["📊 Quality Metrics", "📈 Profiling Results", "✅ Validation Results", "📉 Visualizations"]
        )
        ...
        with tab1:
            ...
            if st.session_state.profile:
                ...
                with col1:
                    st.metric("Quality Score", f"{quality_score:.1%}", delta=None, help="Overall data quality score")

                with col2:
                    st.metric("Completeness", f"{completeness:.1%}", help="Percentage of non-null values")

                with col3:
                    st.metric("Uniqueness", f"{uniqueness:.1%}", help="Average uniqueness across columns")

                with col4:
                    dup_pct = profile.duplicate_percentage
                    st.metric("Duplicates", f"{dup_pct:.1f}%", help="Percentage of duplicate rows")
        ...
        with tab2:
            ...
            if st.session_state.profile:
                ...
                # Column selector
                selected_col = st.selectbox("Select Column to Analyze", options=list(profile.columns.keys()))
                ...
                if selected_col:
                    ...
                    if col_profile.mean is not None:
                        ...
                        if col_profile.outliers_count > 0:
                            st.warning(
                                f"⚠️ {col_profile.outliers_count} outliers detected ({col_profile.outliers_percentage:.1f}%)"
                            )

                    # Top values
                    if col_profile.top_values:
                        st.subheader("Top Values")
                        top_df = pd.DataFrame(col_profile.top_values[:10], columns=["Value", "Count"])
        ...
        with tab4:
            ...
            if st.session_state.profile:
                ...
                null_data = {
                    col_name: col.null_percentage for col_name, col in st.session_state.profile.columns.items()
                }
                null_df = pd.DataFrame(list(null_data.items()), columns=["Column", "Null %"])
                null_df = null_df.sort_values("Null %", ascending=False)

                fig = px.bar(null_df, x="Column", y="Null %", title="Missing Data Percentage by Column")
                st.plotly_chart(fig, use_container_width=True)

                # Uniqueness chart
                st.subheader("Uniqueness by Column")
                unique_data = {
                    col_name: col.unique_percentage for col_name, col in st.session_state.profile.columns.items()
                }
