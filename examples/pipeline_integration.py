...
transactions_df = pd.DataFrame({
    ...
    'payment_method': np.random.choice(['credit_card', 'debit_card', 'paypal', None], 1000, p=[0.5, 0.3, 0.15, 0.05]),
    'source': 'Database'
})
...
transaction_rules = [
    ...
    ValidationRule('status', 'in_list', 'Valid status required', Severity.HIGH,
...
customer_metrics = dqf.monitor.measure_quality(
    ...
    validation_result=customer_validation,
...
customer_passes = (
    customer_metrics.quality_score >= QUALITY_THRESHOLD and
    len(customer_validation.critical_failures) == CRITICAL_FAILURES_ALLOWED
)

transaction_passes = (
    transaction_metrics.quality_score >= QUALITY_THRESHOLD and
    len(transaction_validation.critical_failures) == CRITICAL_FAILURES_ALLOWED
)
...
print("=" * 80)
print(f"Total datasets processed: 2")
...
print(f"Quality metrics tracked: 2")
