...
class ProfileReport:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "high_correlations": [
                {"column1": c1, "column2": c2, "correlation": corr} for c1, c2, corr in self.high_correlations
            ],
...
class DataProfiler:
        ...
        for i in range(len(corr_matrix.columns)):
            for j in range(i + 1, len(corr_matrix.columns)):
                ...
                if abs(corr_value) >= self.correlation_threshold:
                    high_corr.append((corr_matrix.columns[i], corr_matrix.columns[j], float(corr_value)))
    ...
    def _generate_dataset_warnings(
        self, df: pd.DataFrame, columns: dict[str, ColumnProfile], duplicate_percentage: float
    ) -> list[str]:
    ...
    def _generate_recommendations(self, columns: dict[str, ColumnProfile]) -> list[str]:
        ...
        id_candidates = [col.name for col in columns.values() if col.unique_percentage > 95 and col.null_percentage < 5]
        if id_candidates:
