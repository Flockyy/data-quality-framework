...
class DataQualityMonitor:
    ...
    def measure_quality(
        ...
        if validation_result:
            total_failures = sum(
                f.failure_count
                for f in validation_result.get_failures()
            )
            metrics.validation_failures = total_failures
            metrics.validity = 1 - (total_failures / (len(df) * len(validation_result.total_rules))) if len(df) > 0 else 0
        else:
    ...
    def _cleanup_history(self):
        ...
        for dataset_name in self.metrics_history:
            self.metrics_history[dataset_name] = [
                m for m in self.metrics_history[dataset_name]
